...
class Message_Handler:
    ...
    def send(self, payload_data, id, address):
        msg = []
        msg.append(id&0xff)
        msg.append((id>>8)&0xff)
        msg.append((id>>16)&0xff)
        msg.append((id>>24)&0xff)
    ...
    def receive(self):
        ...
        if not self.msg_table.has_key(msg_id):
            print "Unknown Message %d"%(msg_id)
            return

        #Invoke the handler
        self.msg_table[msg_id](data[4:], address, self)
...
class Intercom:
    ...
    def sendTo(self, data):
        if sender_id in self.buddy_list:
            self.msg_handler.send(self, data, self.id, self.address)
        else:
...
def msg_voice_data_handler(msg_data, address, msg_handler):
    voice_data = voice_data_t.voice_data_t()
    voice_data.decode(msg_data)
...
def msg_i_am_handler(msg_data, address, msg_handler):
    ...
    if intercom_name_to_id_table.has_key(i_am.name):
        ...
        msg_handler.send(self, data, i_am_reply_t.i_am_reply_t.MSG_ID, address)

def msg_who_is_handler(msg_data, address, msg_handler):
    who_is = who_is_t.who_is_t()
    who_is.decode(msg_data)
    #Known name?
    if intercom_name_to_id_table.has_key(who_is.name):
        id = intercom_name_to_id_table[who_is.name]
        #Send who-is-reply-back
        who_is_reply = who_is_reply_t()
        who_is_reply.name = who_is.name
        who_is_reply.id = id
        data = who_is_reply.encode()
        msg_handler.send(self, data, who_is_reply_t.who_is_reply_t.MSG_ID, address)

def add_buddy_handler(msg_data, address, msg_handler):
    add_buddy = add_buddy_t.add_buddy_t()
    add_buddy.decode(msg_data)
...
def del_buddy_handler(msg_data, address, msg_handler):
    del_buddy = del_buddy_t.del_buddy_t()
    del_buddy.decode(msg_data)
...
MSG_TABLE = {
    ...
    addy_buddy_t.add_buddy_t.MSG_ID : add_buddy_handler,
