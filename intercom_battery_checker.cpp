...
#define BATTERY_CHECK_FSM_RELEASED 0
#define BATTERY_CHECK_FSM_PRESSED 1

#define MODULE_ID 1500

Intercom_BatteryChecker::Intercom_BatteryChecker(Intercom_ButtonsAndLeds& intercom_buttonsAndLeds) : 
_intercom_buttonsAndLeds(intercom_buttonsAndLeds), _fsm(BATTERY_CHECK_FSM_RELEASED) {
...
void Intercom_BatteryChecker::checkButton(void) {
	...
	if (_fsm == BATTERY_CHECK_FSM_RELEASED) {
		if (buttonPressed) {
			...
			_fsm = BATTERY_CHECK_FSM_PRESSED;
		}
	}
	else {/*fsm=pressed*/
		if (!buttonPressed) {
			Intercom_LedBar ledBar = _intercom_buttonsAndLeds.getLedBar();
			ledBar.setLevel(0);
			_fsm = BATTERY_CHECK_FSM_RELEASED;
		}
	}
}