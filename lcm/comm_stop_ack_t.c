...
static int __comm_stop_ack_t_hash_computed;
static uint64_t __comm_stop_ack_t_hash;

uint64_t __comm_stop_ack_t_hash_recursive(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for (fp = p; fp != NULL; fp = fp->parent)
        if (fp->v == __comm_stop_ack_t_get_hash)
            return 0;

    __lcm_hash_ptr cp;
    cp.parent =  p;
    cp.v = (void*)__comm_stop_ack_t_get_hash;
    (void) cp;
...
{
    if (!__comm_stop_ack_t_hash_computed) {
        __comm_stop_ack_t_hash = (int64_t)__comm_stop_ack_t_hash_recursive(NULL);
        __comm_stop_ack_t_hash_computed = 1;
    }

    return __comm_stop_ack_t_hash;
}
...
{
    int pos = 0, thislen;
    int64_t hash = __comm_stop_ack_t_get_hash();

    thislen = __int64_t_encode_array(buf, offset + pos, maxlen - pos, &hash, 1);
    if (thislen < 0) return thislen; else pos += thislen;

    thislen = __comm_stop_ack_t_encode_array(buf, offset + pos, maxlen - pos, p, 1);
    if (thislen < 0) return thislen; else pos += thislen;
...
{
    return 8 + __comm_stop_ack_t_encoded_array_size(p, 1);
}
...
int comm_stop_ack_t_decode(const void *buf, int offset, int maxlen, comm_stop_ack_t *p)
{
    int pos = 0, thislen;
    int64_t hash = __comm_stop_ack_t_get_hash();
    ...
    thislen = __comm_stop_ack_t_decode_array(buf, offset + pos, maxlen - pos, p, 1);
    if (thislen < 0) return thislen; else pos += thislen;
...
{
    return __comm_stop_ack_t_decode_array_cleanup(p, 1);
}
...
comm_stop_ack_t *comm_stop_ack_t_copy(const comm_stop_ack_t *p)
{
    comm_stop_ack_t *q = (comm_stop_ack_t*) malloc(sizeof(comm_stop_ack_t));
    __comm_stop_ack_t_clone_array(p, q, 1);
    return q;
}

void comm_stop_ack_t_destroy(comm_stop_ack_t *p)
{
    __comm_stop_ack_t_decode_array_cleanup(p, 1);
    free(p);
}
