...
static int __who_is_t_hash_computed;
static uint64_t __who_is_t_hash;

uint64_t __who_is_t_hash_recursive(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for (fp = p; fp != NULL; fp = fp->parent)
        if (fp->v == __who_is_t_get_hash)
            return 0;
...
{
    if (!__who_is_t_hash_computed) {
        __who_is_t_hash = (int64_t)__who_is_t_hash_recursive(NULL);
        __who_is_t_hash_computed = 1;
    }

    return __who_is_t_hash;
}
...
{
    int pos = 0, thislen;
    int64_t hash = __who_is_t_get_hash();

    thislen = __int64_t_encode_array(buf, offset + pos, maxlen - pos, &hash, 1);
    if (thislen < 0) return thislen; else pos += thislen;

    thislen = __who_is_t_encode_array(buf, offset + pos, maxlen - pos, p, 1);
    if (thislen < 0) return thislen; else pos += thislen;
...
{
    return 8 + __who_is_t_encoded_array_size(p, 1);
}
...
int who_is_t_decode(const void *buf, int offset, int maxlen, who_is_t *p)
{
    int pos = 0, thislen;
    int64_t hash = __who_is_t_get_hash();
    ...
    thislen = __who_is_t_decode_array(buf, offset + pos, maxlen - pos, p, 1);
    if (thislen < 0) return thislen; else pos += thislen;
...
{
    return __who_is_t_decode_array_cleanup(p, 1);
}
...
who_is_t *who_is_t_copy(const who_is_t *p)
{
    who_is_t *q = (who_is_t*) malloc(sizeof(who_is_t));
    __who_is_t_clone_array(p, q, 1);
    return q;
}

void who_is_t_destroy(who_is_t *p)
{
    __who_is_t_decode_array_cleanup(p, 1);
    free(p);
}
